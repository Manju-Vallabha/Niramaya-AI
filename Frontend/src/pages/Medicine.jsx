...
function Medicine() {
  ...
  const resultRef = useRef(null); // For auto scroll to typed text
  ...
  const handleFileUpload = (e) => {
    ...
    if (uploadedFile) {
      ...
      if (!validTypes.includes(uploadedFile.type)) {
        setError('Please upload an image or a scanned photo.');
        setStep('error');
        return;
      }
  ...
  const proceedToAnalyze = async () => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/medicine`, formData, {
        headers: {
  ...
  // Auto scroll with typing text, keeping two lines above bottom
  useEffect(() => {
  ...
  return (
    <div className="bg-background flex flex-col items-center justify-center px-4 py-10 min-h-[calc(100vh-4rem)]">
      {/* Logo visible only on start step */}
      {step === 'start' && (
        <motion.div
      ...
      {step === 'start' && (
        ...
        >
          ...
          <p className="text-sm text-gray-600 mt-4 text-center">
            Please upload a image or a scanned photo of your medicine. Supported formats: JPEG, PNG, PDF.
          </p>
        </motion.div>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          ...
          >
            ...
            <p className="text-gray-700 mb-4 text-sm">
              {error || 'The uploaded file is not detectable. Please upload a digital PDF or a scanned image (JPEG/PNG).'}
            </p>
            <button
        ...
        >
          <div className="py-2">
            <div className="flex items-center gap-4 mb-2">
              ...
              <h3 className="text-xl font-bold text-primary">Niramaya AI</h3>
            </div>
          ...
          <div className="text-base font-mono text-ayurBrown whitespace-pre-line">
            <p ref={resultRef} className={isTyping ? 'blinking-text' : ''}>
              {typingText}
            </p>
          </div>

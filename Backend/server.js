...
async function startServer() {
  ...
  app.post('/api/wakeup', (req, res) => {
    try {
      ...
      // For example, ping a database or warm up a cache
      // This is a placeholder; add your logic here
      const wakeupMessage = 'Doctor is ready!';
      ...
      console.error('Error in wakeup endpoint:', error);
      res.status(500).json({
        success: false,
        message: 'Failed to process wakeup request',
      });
    }
  });
...
app.post("/labreport", authenticateToken, upload.single("file"), async (req, res) => {
  ...
  const userEmail = req.user.email;
  const { rows } = await pool.query("SELECT * FROM users WHERE email = $1", [req.user.email]);
      const user = rows[0];
  ...
  try {
    ...
    const health = user.healthrecords || [];
